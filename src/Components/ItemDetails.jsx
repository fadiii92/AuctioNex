            ...
            <div className="flex justify-center mt-4 space-x-2 p-2 bg-gray-200 rounded-md shadow-inner">
              {currentItem.imgUrls.slice(0, 3).map((url, index) => (
                <img
                  ...
                  className={`w-16 h-16 object-cover cursor-pointer border-2 rounded-md transition duration-200 ease-in-out transform ${mainImage === url ? 'border-blue-500 scale-110' : 'border-gray-300 hover:border-blue-400'}`}
                />
              ))}
            </div>
          )}
        ...
        <div className="w-full lg:w-1/2 lg:ml-8 mt-6 lg:mt-0 bg-white p-6 rounded-lg shadow-lg h-full flex flex-col justify-between">
          ...
          {pathname.includes('myItems') ? (
            <>
              ...
              <ConfirmPopup open={isModelOpen} onClose={() => setisModelOpen(false)} deleteItem={handleDelete} />
            ...
            <>
              ...
              <div className="flex justify-between items-center mt-6 mb-4">
                ...
                <button className="px-4 py-2 bg-blue-500 text-white font-semibold rounded hover:bg-blue-600" onClick={() => {
              ...
              <div className="space-y-2">
                {currentItem?.recendBids && Object.keys(currentItem.recendBids).length > 0 ? (
                  Object.values(currentItem.recendBids)
                    ...
                    .reverse()
                    .map((item, index) => (
                      <div key={index} className="bg-gray-100 p-3 rounded-md shadow-md">
                        <p className="text-sm text-gray-700">Bid: ${item.bid}</p>
                      </div>
                    ))
                ) : (
